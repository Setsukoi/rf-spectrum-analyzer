...
from rfsa.fake import FakeResource, fake_resource, synthetic_trace, tone_trace
...
def test_frequency_check_on_fake_analyzer(tmp_path):
    ...
    with Storage(":memory:") as db:
        ...
        assert len(image.read_bytes()) > 500
...
def test_each_preset_completes(tmp_path):
    for chosen in FREQUENCY_PRESETS:
        resource = fake_resource(preset=chosen.name)
        sa = N9020A(resource)
